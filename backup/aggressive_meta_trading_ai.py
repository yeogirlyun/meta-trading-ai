...
# ============================================================================
# ADVANCED ADAPTIVE STRATEGIES (NEW)
# ============================================================================

class MLFilterMomentumStrategy(AggressiveMetaStrategy):
    """
    Uses a Support Vector Machine (SVM) to filter momentum signals.
    The SVM is trained on-the-fly to classify profitable vs. unprofitable market conditions.
    """
    def __init__(self):
        super().__init__("ML-Filter Momentum")
        self.parameters = {
            'fast_ma': 5,
            'slow_ma': 15,
            'svm_lookback': 100,  # How many past periods to use for training the SVM
            'feature_lookback': 20  # Lookback for calculating features like volatility/momentum
        }
        self.model = None
        self.scaler = None
    ...
    def _extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        lookback = self.parameters['feature_lookback']
        features['volatility'] = data['close'].pct_change().rolling(lookback).std()
        features['momentum'] = data['close'].pct_change(lookback)
        features['distance_from_ma'] = (data['close'] - data['close'].rolling(lookback).mean()) / data['close']
        return features.dropna()

    def _train_model(self, data: pd.DataFrame):
        """Trains the SVM model on recent historical data."""
        history = data.tail(self.parameters['svm_lookback'])
        if len(history) < self.parameters['svm_lookback']:
            return  # Not enough data to train

        # 1. Extract Features
        features = self._extract_features(history)
        
        # 2. Create Labels: Was the next 5-period return positive (1) or negative (-1)?
        future_returns = history['close'].shift(-5).pct_change(5)
        labels = np.sign(future_returns).loc[features.index].fillna(0)
        
        # Align features and labels
        aligned_features = features.loc[labels.index]
        
        if len(aligned_features[labels != 0]) < 10:  # Ensure we have enough training examples
            return
            
        # 3. Train the SVM Classifier
        self.scaler = StandardScaler()
        X_scaled = self.scaler.fit_transform(aligned_features[labels != 0])
        y = labels[labels != 0]
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        # Step 1: Train the ML model on the data provided
        self._train_model(data)

        signals = pd.Series(0, index=data.index)
        if not self.is_trained:
            return signals  # Return no signals if model isn't trained

        # Step 2: Generate base signals from a simple crossover
        fast_ma = data['close'].rolling(self.parameters['fast_ma']).mean()
        slow_ma = data['close'].rolling(self.parameters['slow_ma']).mean()
        
        base_long_signal = (fast_ma > slow_ma) & (fast_ma.shift(1) <= slow_ma.shift(1))
        base_short_signal = (fast_ma < slow_ma) & (fast_ma.shift(1) >= slow_ma.shift(1))

        # Step 3: Use the trained ML model to filter signals
        features = self._extract_features(data)
        
        # Get predictions for every point in time
        predictions = pd.Series(self.model.predict(self.scaler.transform(features)), index=features.index)
        
        # Simple approach: use predictions directly where available
        for i in range(len(data)):
            if i < len(predictions):
                if base_long_signal.iloc[i] and predictions.iloc[i] == 1:
                    signals.iloc[i] = 1
                elif base_short_signal.iloc[i] and predictions.iloc[i] == -1:
                    signals.iloc[i] = -1

        return signals.fillna(0)

class HMMRegimeStrategy(AggressiveMetaStrategy):
    """
    Uses a Gaussian Hidden Markov Model (HMM) to identify market regimes
    and applies a different strategy for each detected regime.
    """
    def __init__(self):
        super().__init__("HMM Regime Detection")
        self.parameters = {
            'n_regimes': 3,          # Number of hidden regimes to detect
            'momentum_period': 10,
            'mean_rev_period': 20,
            'volatility_lookback': 20
        }
        self.model = hmm.GaussianHMM(n_components=self.parameters['n_regimes'], 
                                    covariance_type="full", n_iter=200, random_state=42)
        self.is_trained = False
        self.regime_mapping = {}

    def _train_model(self, data: pd.DataFrame):
        """Train the HMM on historical price returns and volatility."""
        returns = data['close'].pct_change().dropna()
        volatility = returns.rolling(self.parameters['volatility_lookback']).std().dropna()
        
        # Remove duplicates and align data
        returns_clean = returns[~returns.index.duplicated(keep='first')]
        volatility_clean = volatility[~volatility.index.duplicated(keep='first')]
        
        # Align data and add small noise to ensure positive definiteness
        training_data = pd.concat([returns_clean, volatility_clean], axis=1).dropna()
        training_data.columns = ['returns', 'volatility']
        
        # Add small noise to ensure positive definiteness
        training_data += np.random.normal(0, 1e-6, training_data.shape)
        
        if len(training_data) > 50:
            try:
                self.model.fit(training_data)
                self.is_trained = True
                self._map_regimes(training_data)
            except:
                # If HMM fails, use simple regime detection
                self.is_trained = False

    def _map_regimes(self, training_data: pd.DataFrame):
        """Identifies the characteristics of each regime."""
        hidden_states = self.model.predict(training_data)
        for i in range(self.parameters['n_regimes']):
            regime_vols = training_data[hidden_states == i]['volatility']
            self.regime_mapping[i] = {'mean_vol': regime_vols.mean(), 'std_vol': regime_vols.std()}
        
        # Sort regimes by volatility to make them identifiable
        sorted_regimes = sorted(self.regime_mapping.items(), key=lambda item: item[1]['mean_vol'])
        self.low_vol_regime = sorted_regimes[0][0]
        self.high_vol_regime = sorted_regimes[-1][0]

    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        self._train_model(data)
        if not self.is_trained:
            # Fallback to simple momentum strategy if HMM fails
            momentum = data['close'].pct_change(self.parameters['momentum_period'])
            return pd.Series(np.sign(momentum), index=data.index).fillna(0)

        # Prepare data for prediction
        returns = data['close'].pct_change()
        volatility = returns.rolling(self.parameters['volatility_lookback']).std()
        
        # Remove duplicates
        returns_clean = returns[~returns.index.duplicated(keep='first')]
        volatility_clean = volatility[~volatility.index.duplicated(keep='first')]
        
        prediction_data = pd.concat([returns_clean, volatility_clean], axis=1).fillna(0)
        prediction_data.columns = ['returns', 'volatility']

        # Detect the current regime for all data points
        regime_predictions = self.model.predict(prediction_data)
        regimes = pd.Series(regime_predictions, index=prediction_data.index)
        # Align with original data index
        regimes = regimes.reindex(data.index, method='ffill').fillna(0)
        signals = pd.Series(0, index=data.index)
        
        # --- Strategy Logic for each Regime ---
        
        # REGIME 1: High Volatility -> Use a momentum/breakout strategy
        momentum = data['close'].pct_change(self.parameters['momentum_period'])
        signals[regimes == self.high_vol_regime] = np.sign(momentum)
        
        # REGIME 2: Low Volatility -> Use a mean-reversion strategy (Bollinger Bands)
        sma = data['close'].rolling(self.parameters['mean_rev_period']).mean()
        std = data['close'].rolling(self.parameters['mean_rev_period']).std()
        upper_band = sma + 2 * std
        lower_band = sma - 2 * std
        
        signals[(regimes == self.low_vol_regime) & (data['close'] < lower_band)] = 1  # Buy
        signals[(regimes == self.low_vol_regime) & (data['close'] > upper_band)] = -1  # Sell

        return signals.fillna(0)

class AdaptiveMultiFactorStrategy(AggressiveMetaStrategy):
    """
    Calculates a weighted score from multiple factors (momentum, trend, volatility)
    and trades when the combined score exceeds an adaptive threshold.
    """
    def __init__(self):
        super().__init__("Adaptive Multi-Factor")
        self.parameters = {
            'short_momentum_period': 5,
            'long_momentum_period': 20,
            'volatility_period': 15,
            'trade_threshold': 1.5,  # The combined score needed to trigger a trade
            # Factor weights - these can also be optimized
            'weight_short_momentum': 0.5,
            'weight_long_momentum': 0.3,
            'weight_volatility': -0.2  # Negative weight for volatility (we prefer lower vol)
        }

    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        # Factor 1: Short-term Momentum (normalized)
        short_momentum = data['close'].pct_change(self.parameters['short_momentum_period'])
        norm_short_momentum = (short_momentum - short_momentum.mean()) / short_momentum.std()

        # Factor 2: Long-term Momentum (normalized)
        long_momentum = data['close'].pct_change(self.parameters['long_momentum_period'])
        norm_long_momentum = (long_momentum - long_momentum.mean()) / long_momentum.std()
        
        # Factor 3: Volatility (normalized and inverted, since low volatility is good)
        volatility = data['close'].pct_change().rolling(self.parameters['volatility_period']).std()
        norm_volatility = (volatility - volatility.mean()) / volatility.std()

        # Calculate combined, weighted score
        combined_score = (
            (norm_short_momentum * self.parameters['weight_short_momentum']) +
            (norm_long_momentum * self.parameters['weight_long_momentum']) +
            (norm_volatility * self.parameters['weight_volatility'])  # Volatility has a negative weight
        ).dropna()
        
        # Generate signals based on the score
        signals = pd.Series(0, index=data.index)
        
        # Align combined_score with signals index
        aligned_score = combined_score.reindex(signals.index, method='ffill').fillna(0)
        
        signals[aligned_score > self.parameters['trade_threshold']] = 1
        signals[aligned_score < -self.parameters['trade_threshold']] = -1
...
class AggressiveMetaTradingAI:
    """Aggressive meta-trading AI system with regime detection and advanced strategies"""
    ...
    def __init__(self):
        ...
        self.high_vol_pool = [
            ...
            BreakoutMomentum(),
            MLFilterMomentumStrategy(),  # NEW: Advanced ML strategy
            HMMRegimeStrategy()          # NEW: Advanced HMM strategy
        ]
        
        self.trending_pool = [
            UltraMomentumAmplification(),
            AcceleratedMACross(),
            AdaptiveMultiFactorStrategy()  # NEW: Advanced multi-factor strategy
        ]
        
        self.ranging_pool = [
            UltraHighFrequencyScalping(),
            ExtremeMeanReversion(),
            MLFilterMomentumStrategy(),    # NEW: Can also work in ranging markets
            AdaptiveMultiFactorStrategy()  # NEW: Can also work in ranging markets
        ]
    ...
    def run_aggressive_meta_system(self, test_period_days: int = 10) -> dict:
        """Run the aggressive meta-trading system with regime detection and advanced strategies"""
