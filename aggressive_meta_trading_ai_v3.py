...
try:
    ...
    print("Warning: arch library not available. GARCH strategy will be disabled.")
...
try:
    ...
    print("Warning: pykalman library not available. Kalman Filter strategy will be disabled.")
...
# ============================================================================
# ULTRA-AGGRESSIVE STRATEGY POOLS
# ============================================================================
...
# ============================================================================
# HIGH-VOLATILITY POOL STRATEGIES (ULTRA-AGGRESSIVE)
# ============================================================================

class UltraVolatilityExploitation(AggressiveMetaStrategy):
    """Ultra-aggressive volatility exploitation with extremely low thresholds"""
    
    def __init__(self):
        super().__init__("Ultra Volatility Exploitation")
        self.parameters = {
            'volatility_threshold': 0.0008,  # 50% lower than v2.2
            'momentum_period': 1,            # Ultra-short period
            'reversal_threshold': 0.0008,    # 50% lower than v2.2
            'max_hold_period': 5,            # Shorter hold period
...
# ============================================================================
# TRENDING POOL STRATEGIES (ULTRA-AGGRESSIVE)
# ============================================================================
...
class UltraMomentumAmplification(AggressiveMetaStrategy):
    ...
    def __init__(self):
        ...
        self.parameters = {
            ...
            'medium_period': 2,             # Shorter medium period
            'long_period': 5,               # Shorter long period
            'momentum_threshold': 0.0004,   # 50% lower threshold
            'acceleration_threshold': 0.0003, # 50% lower acceleration
            'volume_threshold': 1.02,       # Lower volume requirement
            'max_hold_period': 10           # Shorter hold period
        }
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        strong_buy = (
            ...
            (volatility < volatility.rolling(10).mean() * 4)  # More permissive
        )
...
class AcceleratedMACross(AggressiveMetaStrategy):
    """Ultra-fast moving average crossover"""
    
    def __init__(self):
        super().__init__("Accelerated MA Cross")
        self.parameters = {
            'fast_period': 1,               # Ultra-fast period
            'slow_period': 5,               # Shorter slow period
            'momentum_threshold': 0.0005,   # 50% lower threshold
            'volume_threshold': 1.02,       # Lower volume requirement
...
# ============================================================================
# RANGING POOL STRATEGIES (ULTRA-AGGRESSIVE)
# ============================================================================

class UltraHighFrequencyScalping(AggressiveMetaStrategy):
    """Ultra-fast scalping with extremely tight parameters"""
    
    def __init__(self):
        super().__init__("Ultra High-Frequency Scalping")
        self.parameters = {
            'scalp_threshold': 0.0003,      # 50% lower threshold
            'volume_threshold': 1.0,        # Lower volume requirement
            'max_hold_period': 1,           # 1-minute max hold
            'profit_target': 0.0005,        # Tighter profit target
            'stop_loss': 0.0003             # Tighter stop loss
        }
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        scalp_long = (
            ...
            (volume_ratio > self.parameters['volume_threshold']) &
            (volatility < volatility.rolling(10).mean() * 3)  # More permissive
        )
...
class ExtremeMeanReversion(AggressiveMetaStrategy):
    """Ultra-aggressive mean reversion strategy"""
    
    def __init__(self):
        super().__init__("Extreme Mean Reversion")
        self.parameters = {
            'mean_period': 3,               # Shorter mean period
            'deviation_threshold': 0.001,   # 50% lower threshold
            'volume_threshold': 1.02,       # Lower volume requirement
            'max_hold_period': 5            # Shorter hold period
        }
...
class GARCHVolatilityForecastingStrategy(AggressiveMetaStrategy):
    """
    Uses a GARCH model to forecast volatility and trades when the forecast
    is within an optimal range.
    """
    def __init__(self):
        super().__init__("GARCH Volatility Forecasting")
        self.parameters = {
            'forecast_horizon': 5,      # Forecast 5 minutes ahead
            'optimal_vol_min': 0.0001,  # Minimum forecasted volatility to trade
            'optimal_vol_max': 0.0005,  # Maximum forecasted volatility
            'momentum_period': 5
        }

    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        if not ARCH_AVAILABLE or len(data) < 50:
        ...
        try:
            ...
            # Forecast volatility
            forecast = res.forecast(horizon=self.parameters['forecast_horizon'])
            predicted_vol = np.sqrt(forecast.variance.iloc[-1]) / 100
            
            signals = pd.Series(0, index=data.index)
            momentum = data['close'].pct_change(self.parameters['momentum_period'])
            
            # Generate signals based on the forecast
            if self.parameters['optimal_vol_min'] < predicted_vol.mean() < self.parameters['optimal_vol_max']:
...
class KalmanFilterAdaptiveMA(AggressiveMetaStrategy):
    """
    Uses a Kalman Filter to create a dynamically adjusting moving average.
    """
    def __init__(self):
        super().__init__("Kalman Filter Adaptive MA")
        self.parameters = {
            'observation_covariance': 0.01
        }

    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        if not KALMAN_AVAILABLE or len(data) < 20:
        ...
        try:
            kf = KalmanFilter(
                ...
                n_dim_obs=1,
                observation_covariance=self.parameters['observation_covariance']
            )
            state_means, _ = kf.filter(data['close'].values)
            kalman_ma = pd.Series(state_means.flatten(), index=data.index)
            
            signals = pd.Series(0, index=data.index)
            signals[data['close'] > kalman_ma] = 1
            signals[data['close'] < kalman_ma] = -1
...
class AggressiveStrategySelector:
    """Enhanced strategy selector with predictive features"""
    ...
    def extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # NEW: Enhanced predictive features
...
class AggressiveMetaTradingAI:
    """Ultra-aggressive meta-trading AI system with enhanced intelligence"""
    
    def __init__(self):
        # Initialize strategy pools with ultra-aggressive parameters
        self.high_vol_pool = [
            UltraVolatilityExploitation(),
            BreakoutMomentum(),
            GARCHVolatilityForecastingStrategy() if ARCH_AVAILABLE else None
        ]
        
        self.trending_pool = [
            UltraMomentumAmplification(),
            AcceleratedMACross(),
            KalmanFilterAdaptiveMA() if KALMAN_AVAILABLE else None
        ]
        ...
        self.high_vol_pool = [s for s in self.high_vol_pool if s is not None]
        self.trending_pool = [s for s in self.trending_pool if s is not None]
        self.ranging_pool = [s for s in self.ranging_pool if s is not None]
    ...
    def run_aggressive_meta_system(self, test_period_days: int = 10) -> dict:
        ...
        # Run ultra-aggressive meta system with detailed reporting
        print(f"\nRunning ultra-aggressive meta-trading system...")
        results = {
            'hourly_performance': [],
            'daily_performance': [],
            'selected_strategies': [],
